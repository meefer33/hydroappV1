...
export default function PuckLayoutContent({
  config,
  contentData = {},
  saveMeta = {},
  type = '',
  ...
  const {saveSection} = useInsertSection();
    ...
    >
      <Puck
        onAction={(action, appState, prevAppState) => {
          //console.log('onAction', action, appState, prevAppState);
          if (action.type === 'insert' && action.componentType === 'Section') {
            //insert into sections metaobjects and update content
            //saveSection(action,appState)
          ...
          if (action.type === 'remove' && action.zone === 'root:main-content') {
            ...
            const removeId =
              prevAppState?.data?.zones['root:main-content'][action.index].props
                ?.id;
      ...
      >
        <LoadSectionModal
          loadSectionOpened={loadSectionOpened}
          loadSectionClose={loadSectionClose}
          loadSectionId={loadSectionId}
        />
